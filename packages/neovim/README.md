...
### 로컬 → 레포 백업
```sh
rm -rf packages/neovim/lazyvim/lua
cp -R ~/.config/nvim/lua packages/neovim/lazyvim/
```
- 레포에서 삭제 후 복사하므로 Git diff를 반드시 확인하세요.

### 레포 → 로컬 배포 (수동)
```sh
cp -R packages/neovim/lazyvim/lua ~/.config/nvim/lua
```
...
### 로컬 → 레포 백업
```sh
rm -rf packages/neovim/nvchad/lua/custom
cp -R ~/.config/nvim/lua/custom packages/neovim/nvchad/lua/
```
...
cp -R packages/neovim/nvchad/lua/custom ~/.config/nvim/lua/
```
