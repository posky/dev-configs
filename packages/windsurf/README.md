...
cp packages/windsurf/settings.json "$DEST"
```
- 기존 파일이 있다면 `cp "$DEST" "$DEST".backup.$(date +%Y%m%d%H%M)`으로 백업 후 덮어쓰세요.

## 로컬 → 레포 백업
```sh
cp "$HOME/Library/Application Support/Windsurf/User/settings.json" packages/windsurf/settings.json
```

## 차이 비교
```sh
code --diff packages/windsurf/settings.json "$HOME/Library/Application Support/Windsurf/User/settings.json"
```
