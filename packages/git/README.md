...
1. 템플릿과 설정 파일을 홈 디렉터리로 배치합니다.
   ```sh
   cp packages/git/.gitconfig ~/.gitconfig           # 기존 설정과 병합이 필요하면 적절히 조정
   cp packages/git/.gitmessage.txt ~/.gitmessage.txt
   ```
...
## 백업 방법 (로컬 → 레포)
```sh
cp ~/.gitmessage.txt packages/git/
```
...
```
- 파일을 복사할 때는 `Copy-Item packages\\git\\.gitmessage.txt "$env:USERPROFILE\\.gitmessage.txt"`를 사용합니다.
