...
1. 레포 → 로컬 적용
   ...
   DEST="$(lazygit -cd)/config.yml"
    cp packages/lazygit/config.yml "$DEST"
   ```
2. 로컬 → 레포 백업
   ```sh
    cp "$(lazygit -cd)/config.yml" packages/lazygit/
   ```
3. 설정 비교
   ```sh
   LOCAL="$(lazygit -cd)/config.yml"
    code --diff packages/lazygit/config.yml "$LOCAL"  # VS Code
   # 또는
    vim -d packages/lazygit/config.yml "$LOCAL"
   ```

## 참고
- 커밋 템플릿 프롬프트는 `packages/git/.gitmessage.txt` 구조와 동일하게 유지해야 UI가 일치합니다.
